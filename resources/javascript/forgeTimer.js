// Comprehensive forge times database (in milliseconds)
// Note: This includes all 116+ forgeable items
function getBaseForgeTime(id, type) {
    ...
    return typeTimes[type] || 72000000; // default 20 hours
}
...
async function loadForgeData() {
    try {
        ...
        const profiles = await getProfiles(apiKeyVar, uuidVar);
        const profile = profiles.find(p => p.members[uuidVar]?.selected) || profiles[0];
...
function updateForgeDisplay() {
    ...
    for (let i = 1; i <= 7; i++) {
        ...
        if (!slot) {
            ...
            const itemName = slot.id.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, l => l.toUpperCase());
...
function startForgeTimer() {
    console.log("Starting forge timer");
